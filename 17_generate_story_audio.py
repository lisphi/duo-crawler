...
def generate_story_mp3_and_lrc(transcription, stories_path, storie_dir_name, rewrite):
    ...
    for i in range(len(transcription['lines'])):
        ...
        reciter_name = line['reciterName']
        if len(reciter_name) > 0:
            reciter_name = f"({reciter_name}) "

        lyrics.append(f"[{format_duration(total_duration)}]{reciter_name}{line['content']['text']}")
...
def generate_story_lrc(transcription, stories_path, storie_dir_name, rewrite):
    target_lrc_filename = f"{stories_path}/{storie_dir_name}/{storie_dir_name[:4]}_{storie_dir_name[11:].split('_')[0]}.lrc"

    if os.path.exists(target_lrc_filename):
        if rewrite:
            os.remove(target_lrc_filename)
        else:
            print(f"{target_lrc_filename} exists")        
...
def main():
    ...
    for i in range(len(storie_dir_names)):
        ...
        # if 'good-morn' not in storie_dir_name:
        #     continue
        ...
        generate_story_mp3_and_lrc(transcription, stories_path, storie_dir_name, False)
        # generate_story_lrc(transcription, stories_path, storie_dir_name, False)
